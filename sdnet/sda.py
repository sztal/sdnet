...
class SDA:
    ...
    def conf_model(self, simplify=True, sparse=True, p_rewire=None, directed=None,
                   self_loop_p=10**-6, multiple_edges_p=10**-6):
        ...
        P = self.P.copy()
        np.fill_diagonal(P, self_loop_p)
        # Assign zero probabilities to impossible edges
        ...
        while n_stubs > 0:
            ...
            A[i, j] += 1
            if P[i, j] > 0:
                W[i] += multiple_edges_p - P[i, j]
                P[i, j] = multiple_edges_p
            if not directed:
                A[j, i] += 1
                if P[j, i] > 0:
                    W[j] += multiple_edges_p - P[j, i]
                    P[j, i] = multiple_edges_p
        # Final processing
        if simplify:
